...
class M2M100ForConditionalGeneration(M2M100PreTrainedModel):
    ...
    def forward(
        ...
        #     decoder_attention_mask[decoder_input_ids == self.config.pad_token_id] == 0              # 1表示不mask，0表示mask
        outputs = self.model(
            input_ids,
            attention_mask=attention_mask,
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # elif decoder_input_ids.shape[-1] == 1 and hasattr(self.generation_config, "forced_eos_token_id")    \
        #     and self.generation_config.forced_eos_token_id != None:
        #     batch_size, seq_len = decoder_input_ids.shape
        #     seq_len += 1
        #     token_ids = torch.full((batch_size, seq_len),
        #                            self.generation_config.forced_eos_token_id).to(device=decoder_input_ids.device,
        #                                                                           dtype=decoder_input_ids.dtype)
        #     token_ids[:, :1] = decoder_input_ids[:, :]
